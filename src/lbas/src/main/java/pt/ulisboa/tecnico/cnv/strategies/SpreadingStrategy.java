...
public class SpreadingStrategy implements VmSelectionStrategy {
    @Override
    public List<String> selectVms(Map<String, Worker> workers, long requestComplexity, long avgLoad) {
        return workers.entrySet().stream()
                ...
                // Sort by current load descending (least loaded first)
                .sorted(Map.Entry.comparingByValue(Comparator.comparingLong(Worker::getCurrentLoad)))
