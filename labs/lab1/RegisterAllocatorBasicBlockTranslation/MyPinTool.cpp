...
KNOB< string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "RegisterAllocatorOut.out", "specify profile file name");
...
VOID ComputeGlobalStats()
{
    ...
    {
        struct GLOBALSTATS *blockStats = (GLOBALSTATS *)&(*bi)->_localStats; // local stats for this bbl
...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        BBLSTATS* bblstats = new BBLSTATS(stats, BBL_Address(bbl));
...
VOID Fini(int, VOID* v)
{
    ...
    *out << "#\n"
    ...
    // Output per-basic-block register usage statistics
    *out << "\n# Per-Basic-Block Register Usage:\n";

    for (list<const BBLSTATS*>::iterator bi = statsList.begin(); bi != statsList.end(); bi++)
    {
        const BBLSTATS* bblstat = *bi;
        
        // Print header for the basic block, using its address
        *out << "## Basic Block at " << StringFromAddrint(bblstat->_bbl_addr) << "\n";

        // Iterate through registers and print the register usage statistics per block
        for (UINT32 i = 0; i < MAX_REG; i++)
        {
            if (bblstat->_localStats.reg_r[i] == 0 && bblstat->_localStats.reg_w[i] == 0) continue;

            *out << decstr(i, 3) << " " << ljstr(REG_StringShort(REG(i)), 15)
                    << decstr(bblstat->_localStats.reg_r[i], 12)
                    << decstr(bblstat->_localStats.reg_w[i], 12) << endl;
        }
    }
